/*
 * ESP8266_RTC_initFrom_NTP.ino
 * A sketch showing RTC module (DS1307)'s initialization from a NTP time source
 *  Adapted from several sources (in README.md)
 * 
 * by Yogeshwar Bala (github.com/Blend3rman)
 */
...
// Calcluate offset from GMT in seconds
...
void setup()
{
  ...
  delay(250);
  Serial.println("RTC init from NTP Example");
  ...
  Serial.print("RTC init done! Time from module: ");
  char rtcTime[] = "hh:mm:ss DD.MM.YYYY";
  rtc.now().toString(rtcTime);
...
void loop()
{
  if (rtc.now().unixtime() != prevDisplay) { //update the display only if time has changed
...
uint32_t getNtpTime()
{
  ...
  while (millis() - beginWait < 3000UL) {
    ...
    if (size >= NTP_PACKET_SIZE) {
      ...
      // Add NTP server response delay
      fractionalPart += (millis() - beginWait)/2;
      ...
      // We return the result at the next second deadon
      secsSince1900++;
      delay(1000 - fractionalPart);
      return secsSince1900 - 2208988800UL + timeZoneOffsetSeconds;
    }
  }
